...
const OrderTimer: FC<{ date: string, limitMinutes: number, onExpire: () => void }> = ({ date, limitMinutes, onExpire }) => {
    ...
    return (
        <div className="mt-2 p-2 bg-primary/5 dark:bg-primary/10 rounded-xl border border-primary/10">
            <div className="flex justify-between items-center mb-1">
                <span className="text-[9px] font-black text-primary/60 uppercase tracking-widest">Auto-Refund Timer</span>
...
const PurchaseCard: FC<{ purchase: Purchase, texts: any, appSettings: AppSettings, index: number, onDelete: (p: Purchase) => void, onAutoRefund: (p: Purchase) => void }> = ({ purchase, texts, appSettings, index, onDelete, onAutoRefund }) => {
...
const MyOrdersScreen: FC<MyOrdersScreenProps> = ({ user, texts, appSettings, adCode, adActive }) => {
    ...
    const handleAutoRefund = async (p: Purchase) => {
        ...
        // Delete Order (Automated removal)
        ...
        await push(notifRef, {
            ...
            message_bn: `আপনার #${p.id} অর্ডারটি (৳${p.offer?.price}) নির্দিষ্ট সময়ে প্রসেস না হওয়ায় অটোমেটিক রিফান্ড করা হয়েছে।`,
            timestamp: Date.now(),
            type: 'failed',
            targetUid: user.uid,
            isAuto: true
            ...
            <div className="max-w-lg mx-auto">
                <div className="relative bg-gradient-to-r from-primary to-secondary p-5 rounded-2xl text-white shadow-xl mb-6 overflow-hidden animate-smart-slide-down flex items-center justify-between">
                    ...
                    <div className="relative z-10">
                        <p className="text-[10px] font-bold text-white/80 uppercase tracking-widest mb-1">{texts.totalOrder}</p>
                        <h3 className="text-2xl font-extrabold text-white tracking-tight">{orders.length}</h3>
                        <p className="text-[9px] text-white/70 mt-0.5 font-medium">History Records</p>
                    </div>
                                    ...
                                    <div 
                                        ...
                                        className={`keep-animating transition-all duration-500 ease-in-out transform
                                            ${isExiting 
                                                ...
                                                : 'opacity-100 translate-x-0 max-h-[1000px] pb-4'}
                                        `}
                                    >
                                        <PurchaseCard 
                                            purchase={p} 

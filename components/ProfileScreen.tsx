...
const ProfileScreen: FC<ProfileScreenProps> = ({ user, texts, onLogout, setActiveScreen, theme, setTheme, language, setLanguage, appSettings }) => {
    ...
    // Add Quiz if Enabled
    if (appSettings?.isQuizEnabled !== false) {
        menuItems.push({ label: texts.ffQuiz, icon: GamepadIcon, action: () => setActiveScreen('gamerTest') });
    }

    menuItems.push({ label: texts.contactUs, icon: ContactUsIcon, action: () => setActiveScreen('contactUs') });
    menuItems.push({ label: texts.changePassword, icon: KeyIcon, action: () => setActiveScreen('changePassword') });

    // Add Ranking if Enabled
    if (appSettings?.visibility?.ranking !== false) {
        const rankingIndex = menuItems.findIndex(i => i.label === texts.ffQuiz);
        const insertAt = rankingIndex !== -1 ? rankingIndex + 1 : 3;
        menuItems.splice(insertAt, 0, { label: texts.ranking, icon: TrophyIcon, action: () => setActiveScreen('ranking') });
    }
